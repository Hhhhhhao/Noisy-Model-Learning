...
def maybe_dictionarize(batch):
    ...
    if len(batch) == 2:
        batch = {'images': batch[0], 'labels': batch[1], 'metadata': [None]}
    elif len(batch) == 3:
...
def get_features(is_train, image_encoder, dataset, device):
    ...
    if image_encoder.cache_dir is not None and len(cached_files) > 0:
        ...
        print(f'Did not find cached features. Building from scratch.')
...
class FeatureDataset(Dataset):
    def __init__(self, is_train, image_encoder, dataset, device, return_dict):
...
def get_dataloader(dataset, is_train, args, image_encoder=None, return_dict=True):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, args.device, return_dict)
        dataloader = DataLoader(feature_dataset, batch_size=args.batch_size, shuffle=is_train, drop_last=False, pin_memory=True)
    else:
...
def sample_num_shot(samples, labels, num_classes, sample_ratio):
    ...
    for i in range(num_classes):
        ...
        num_shots = int(len(masked_samples) * sample_ratio)
        rand_idxs = np.random.choice(range(len(masked_samples)), num_shots, replace=False)
        # if num_shots <= len(masked_samples):
        #     rand_idxs = np.random.choice(range(len(masked_samples)), num_shots, replace=False)
        # else:
        #     print("shots", i, num_shots, len(samples), len(masked_samples))
        #     rand_idxs = np.random.choice(range(len(masked_samples)), num_shots, replace=True)  #### e.g. domainnet in unbalanced
        
        # rand_idxs = np.random.choice(range(len(masked_images)), len(masked_images), replace=False)
        selected_samples.extend(masked_samples[rand_idxs])
...
def sample_noise_data(samples, labels, num_classes, noise_ratio):
    samples, targets = np.array(samples), np.array(labels)
